...
function ExerciseClip({ clipPath, exerciseName }: { clipPath: string; exerciseName: string }) {
  ...
  // Lazy load: only load video src when visible in viewport
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      />
      {/* Play/Pause overlay — full area is touch target */}
...
export default function WorkoutDetail2025Client({ dateStr }: { dateStr: string }) {
  const { startDate } = useWorkout()
  const [manifest, setManifest] = useState<ClipManifest | null>(null)

  // Load clip manifest with retry for poor networks
  useEffect(() => {
    let attempt = 0
    const load = () => {
